...
20. **Shopify-магазины и сайты с публичным product JSON API — HTTP-адаптер, не Firecrawl.** Если у источника есть `GET /products/{handle}.json`, `GET /collections/{handle}/products.json` или UCP/MCP endpoint (`/api/ucp/mcp`) — scrape через Firecrawl нарушает принцип «API first». Признаки: robots.txt с комментарием про Shopify/UCP, наличие `/agents.md` с задокументированными endpoint'ами. Правильный путь: HTTP-адаптер без Firecrawl на основе `_http_base.py` (готов в Phase 8 — `HttpSourceAdapter` Protocol + `fetch_via_http` + `run.py` dispatch по `getattr(adapter, "kind", KIND_FIRECRAWL)`). Технический онбординг через готовый Protocol теперь возможен, но `/agents.md` сам по себе НЕ освобождает от ToS-проверки (см. Lesson #21). Пример успешного HTTP-онбординга: `dummyjson.com` (Phase 8); пример отказа по ToS несмотря на `/agents.md`: `jovianarchive.com`.

21. **`/agents.md` ≠ ToS-разрешение на automated data collection.** Сайт может публиковать `/agents.md` как commerce facilitator для shopping agents (UCP/MCP с buyer-approved payment), но в общих ToS запрещать automated data extraction для построения внешних аналитических баз. Перед онбордингом источника с `/agents.md` — отдельно прочитать ToS и убедиться что НАШ use case (analytical catalog, не shopping workflow) разрешён. Phase 8 example: `jovianarchive.com` — `/agents.md` явно приглашает UCP/MCP shopping agents и документирует `GET /products/{handle}.json` для read-only browsing; общий ToS блокирует «systematic or automated data collection activities... use such content to construct any kind of database... without express written consent». Backlog-метка с Phase 7 снята, не возвращаемся без письменного разрешения owner'а.

22. **HTTP-адаптеры НЕ конвертируют JSON primitive → Decimal в `parse_response`.** `record_attempt` пишет `raw_payload` через `json.dumps` ДО Pydantic-валидации (`raw_content` — append-only audit). Decimal в payload даёт `TypeError: Object of type Decimal is not JSON serializable` и роняет batch на первом fetch'е. Conversion делает Pydantic schema-validator на стадии `validate_extracted` — внутренне применяет `Decimal(str(value))`, precision сохраняется. Symmetric с Firecrawl flow: LLM возвращает float, Pydantic делает str-coercion downstream. Phase 8 example: `dummyjson_com` first paid batch crash → hotfix `fix/dummyjson-price-keep-float-for-json-audit`. Lock-in: integration test `test_parse_response_output_validates_as_product_with_decimal_price` в `tests/sources/test_dummyjson_com.py` проверяет full chain adapter → `Product.model_validate` → `Decimal("9.99")` без precision loss. Pattern для следующих HTTP-адаптеров: `parse_response` возвращает JSON primitives (`float | None`, `int | None`, `str | None`, `bool`); все Decimal/datetime/etc. — забота схемы.
...
2. **Последний** `HANDOFF_PHASE<n>_COMPLETE.md` — авторитативный снапшот текущего состояния (DB-counts, baseline, открытые TODO). На момент этого обновления: **`HANDOFF_PHASE8_COMPLETE.md`** (96 canonical, MDN 34 / Python 27 / Anthropic 19 / ScrapethisSite 6 / DummyJSON 10; baseline 183; TODO #3 открыт, TODO #8 foundation закрыт + расширения остались, TODO #9 новый — dummyjson seed expansion). Обновляй ссылку при закрытии каждого major-цикла.
3. Архивные handoff'ы по убыванию давности — для исторического контекста архитектуры, ролей, запретов:
   - `HANDOFF_PHASE7_COMPLETE.md` — scrapethissite_com онбординг, user manual, отказы openai.com и jovianarchive (предварительный, до ToS-check).
